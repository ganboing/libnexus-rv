...
struct rv_inst_block {
    ...
    inline virtual int print_pre(FILE *fp) {
        return fprintf(fp, "%s %s", insn->mnemonic, insn->op_str);
    }
    inline virtual int print_post(FILE* fp) {
        return 0;
    }
protected:
...
struct rv_ib_cond_branch : rv_ib_dir_jmp {
    ...
    uint64_t retire(nexusrv_trace_decoder *decoder) override;
    int print_post(FILE *fp) override;
protected:
    mutable bool taken;
};
...
struct rv_ib_indir_call : rv_ib_indir_jmp {
    using rv_ib_indir_jmp::rv_ib_indir_jmp;
...
struct rv_ib_ret : rv_ib_indir_jmp {
    using rv_ib_indir_jmp::rv_ib_indir_jmp;
    inline ~rv_ib_ret() {}
    uint64_t retire(nexusrv_trace_decoder *decoder) override;
    int print_post(FILE* fp) override;
...
struct rv_ib_co_swap : rv_ib_ret {
    using rv_ib_ret::rv_ib_ret;
